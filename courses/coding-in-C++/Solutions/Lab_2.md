...
#include <string>
#include <iomanip>
using namespace std;  // don't use "using"

class Article { //write into seperate header file
public:
    string name;      // make the variables private
    double price;
    int stock;
    string* category; //pointer makes no sense
    int id;

    Article(string name, double price, int stock, string category, int id) {
        name = name;      // you can initalize them in an initialiser list
        price = price;      //"
        stock = stock;      //"
        this->id = id;      //" //every article has the same id
        this->category = new string;     
        *this->category = category;  
    }

    void setPrice(double price) {
        price = price; // initialize via initialiser list
    }

    void sell(int amount) {
        stock = stock - amount; //this->stock -= amount;
      //error analysis for selling more than stock or selling negative amount
    }

    void restock(int amount) {
        this->stock += amount;   //error analysis for negative restock
    }

    double applyDiscount(double percent) {
        price = price - price * percent / 100;  //this-> price
        return price;  // error analysis for more than 100% discount or negative discount
    }

    double getPrice() {
        return price;     // make function const
    }

    bool isAvailable() {  //intitialize after //make function const
        if (stock > 0)  //missing {} //return stock>0;
            return true;
        else  
            return false;
    }

    void printInfo() {   // initialize after class
...
int main() {
    ...
    a.sell(15);
    a.restock(-5);
    a.price = -100; //no such function so changing price directly
    a.applyDiscount(150);
