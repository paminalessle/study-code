...
{
    if (intensity > 30)     //und falsche limits gesetzt
    {
...
{
    repository.save(player); //why make 2 functions for the same task?
}
...
{
    notifier.send(player, message); //why make two functions for the same task?
}
...
void FootballManager::select_strategy(const std::string &strategy)
{
    ...
    else
    {
        ...
        //make a base class Strategy and then seperate classes for each strategy
        //that way a player can easily change strategies and one can further adapt the strategies
    }
}
