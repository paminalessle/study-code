...
constexpr int MAX_ACCELERATE_VALUE_kmh = 300;

Vehicle::Vehicle(const std::string &vehicle_model)
    : model(vehicle_model), speed_kmh(0.0), steering_angle(0.0), lane_offset_m(0.0), brake_light_on(false) {}

void Vehicle::accelerate(double amount_kmh) {
    if (amount_kmh >= MAX_ACCELERATE_VALUE_kmh) {
        std::cout << "Invalid acceleration. Exceeds the limit.\n";
    }
...
void Vehicle::brake(double amount_kmh) {
    if (amount_kmh <= 0.0) {
        std::cout << "Not a valid braking value\n";
        return;
    }
...
const std::string &Vehicle::get_model() {
    return model;
}
